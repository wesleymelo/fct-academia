...
function retorna(id, nome)
        {
           ...
           window.self.close();
		   window.opener.document.getElementById('idGraduacao').value = id;
		   window.opener.document.getElementById('graduacao').value = nome;
        }
</script>
...
<body>
		...
		<form action="buscaGraduacao.do" method="post">
				<fieldset>
					...
					<table id="myTable" class="tablesorter">
						...
						<tbody>
							<c:forEach var="gradu" items="${graduacoes}" varStatus="i" begin="${inicio}" end="${fim}" >
								<tr>
									<td>${gradu.idGraduacao}</td>
									<td><a href="javascript:retorna('${gradu.idGraduacao}', '${gradu.descricao}');">${gradu.descricao}</a></td>
									<td>${gradu.modalidadeString}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
		...
		<div class="pagination">
			<a href="${pageContext.request.contextPath}/view/admin/graduacao/listaGraduacoes.do?pag=1" class="button"><span><img
			...
			<a href="${pageContext.request.contextPath}/view/admin/graduacao/listaGraduacoes.do?pag=<c:if test="${pag > 1}">${pag - 1}</c:if><c:if test="${pag == 1}">1</c:if>"
					class="button"><span>
			...
			<div class="numbers">
				...
				<c:forEach var="i" begin="1" end="${numPags}">
					...
					<c:if test="${i != pag}">
						<a href="${pageContext.request.contextPath}/view/admin/graduacao/listaGraduacoes.do?pag=${i}"><c:out value="${i}" /></a>
					</c:if>
					<c:if test="${i < numPags}">
						<span>|</span>
					</c:if>	
			...
			<a href="${pageContext.request.contextPath}/view/admin/graduacao/listaGraduacoes.do?pag=<c:if test="${pag < numPags}">${pag + 1}</c:if> <c:if test="${pag == numPags}">${pag}</c:if>" class="button"><span><fmt:message key="proximo"/> <img
					src="${pageContext.request.contextPath}/view/images/arrow-000-small.gif"
					height="9" width="12" alt="Next" /></span>
			</a> 
					
			<a href="${pageContext.request.contextPath}/view/admin/graduacao/listaGraduacoes.do?pag=<c:out value="${numPags}"/>" class="button last"><span><fmt:message key="ultimo"/><img
