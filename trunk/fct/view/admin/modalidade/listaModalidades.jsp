...
<div class="container_12">
	...
	<div class="grid_12">
		...
		<div class="bottom-spacing">
			...
			<form action="buscaModalidade.do" method="post">
				<fieldset>
		...
		<c:if test="${fn:length(modalidades) % tamPag == 0}">
			<c:set var="numPags" value="${fn:length(modalidades) / tamPag }" />
		</c:if>
		<c:if test="${fn:length(modalidades) % tamPag != 0}">
			<c:set var="numPags" value="${(fn:length(modalidades) / tamPag)+1}" />
		</c:if>


		<div class="pagination">
			<a href="${pageContext.request.contextPath}/view/admin/modalidade/listaModalidades.do?pag=1" class="button"><span><img
			...
			<a href="${pageContext.request.contextPath}/view/admin/modalidade/listaModalidades.do?pag=<c:if test="${pag > 1}">${pag - 1}</c:if><c:if test="${pag == 1}">1</c:if>"
					class="button"><span>
			...
			<div class="numbers">
				...
				<c:forEach var="i" begin="1" end="${numPags}">
					...
					<c:if test="${i != pag}">
						<a href="${pageContext.request.contextPath}/view/admin/modalidade/listaModalidades.do?pag=${i}"><c:out value="${i}" /></a>
					</c:if>
					<c:if test="${i < numPags}">
						<span>|</span>
					</c:if>	
			...
			<a href="${pageContext.request.contextPath}/view/admin/modalidade/listaModalidades.do?pag=<c:if test="${pag < numPags}">${pag + 1}</c:if> <c:if test="${pag == numPags}">${pag}</c:if>" class="button"><span><fmt:message key="proximo"/> <img
					src="${pageContext.request.contextPath}/view/images/arrow-000-small.gif"
					height="9" width="12" alt="Next" /></span>
			</a> 
					
			<a href="${pageContext.request.contextPath}/view/admin/modalidade/listaModalidades.do?pag=<c:out value="${numPags}"/>" class="button last"><span><fmt:message key="ultimo"/><img
