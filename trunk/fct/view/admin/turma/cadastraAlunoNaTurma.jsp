...
<c:if test="${! empty param.show && param.show == true}">
	<div id="dialog-message" title="<fmt:message key="visualizaAluno"/>">
		...
		<b> <fmt:message key="nome" />:
		</b>${aluno.nome} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><fmt:message
				key="nascimento" />: </b>${aluno.dateNascimentoString}<br /> <b> <fmt:message
				key="cpf" />:
		</b>${aluno.cpf} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><fmt:message
				key="sexo" />: </b>${aluno.sexo.descricao}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<b> <fmt:message key="dataCadastro" />:
		</b>${aluno.dateCadastroString}<br /> <b><fmt:message key="status" />:
		</b>${aluno.situacao}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>
			<fmt:message key="altura" />:
		</b>${aluno.altura} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><fmt:message
				key="peso" />: </b>${aluno.peso} kg<br />
		<br />
		...
		<b> <fmt:message key="logradouro" />:
		</b>${aluno.endereco.enderecoResidencial}
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><fmt:message
				key="cidade" />: </b>${aluno.endereco.cidade}<br /> <b> <fmt:message
				key="bairro" />:
		</b>${aluno.endereco.bairro}
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><fmt:message
				key="complemento" />: </b>
		<c:out value="${aluno.endereco.complemento}: " default="vazio" />
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b> <fmt:message
				key="uf" />:
		</b>${aluno.endereco.uf}<br /> <b><fmt:message key="cep" />: </b>${aluno.endereco.cep}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<b> <fmt:message key="numero" />:
		</b>${aluno.endereco.numero}<br />
		<br />
		...
		<table>
			<thead>
				<c:forEach var="telefone" items="${aluno.telefones}">
					<tr>
						<td><b><fmt:message key="numero" />:</b> (${telefone.ddd})
							${telefone.numero}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						</td>
						<td><b><fmt:message key="tipo" />:</b>
							${telefone.tipo.descricao}</td>

					</tr>
				</c:forEach>
			</thead>
		</table>
		<b> <fmt:message key="email" />:
		</b>${aluno.email}<br />
		<br />
...
<div class="container_12">
	...
	<div class="grid_6">
		...
		<div class="module">
			<h2>
				<span><fmt:message key="inserirAlunoTurma" />
					<c:out value=" ${turma.nome}" /></span>
			</h2>


			<div class="module-body">
				<form action="cadastraAlunoNaTurma.do" method="post">
					<input type="hidden" name="idAluno" id="idAluno" /> <input
						type="hidden" name="idTurma" id="idTurma" value="${param.id}" />
					<p>
						<label><fmt:message key="aluno" /></label> <input type="text"
							value="${param.aluno}" class="input-medium" name="aluno"
							id="aluno" readonly="readonly" value="${aluno}" /><a
							href="javascript:abrir('buscaAlunoTurma.do')"> <fmt:message
								key="buscar" />
						</a>

						<c:if test="${not empty erros['erroturmacheia'] }">
							<span class="notification-input ni-error"><fmt:message
									key="turmacheia_invalida" /></span>
						</c:if>

						<c:if test="${not empty erros['erroaluno'] }">
							<span class="notification-input ni-error"><fmt:message
									key="aluno_invalido" /></span>
						</c:if>

						<c:if test="${not empty erros['erroaluno_duplicado'] }">
							<span class="notification-input ni-error"><fmt:message
									key="aluno_duplicado" /></span>
						</c:if>
					</p>

					<fieldset>
						<input class="submit-green" type="submit"
							value="<fmt:message key="inserir"/>" /> <input
							class="submit-gray" type="reset"
							value="<fmt:message key="bt_limpar"/>" />
					</fieldset>
				</form>
			</div>
	...
	<div class="grid_6">
		...
		<div class="module">
			<h2>
				<span><fmt:message key="turma" /></span>
			</h2>

			<div class="module-body">
				<b><fmt:message key="nome" />: </b>${turma.nome} <br /> <b><fmt:message
						key="professor" />: </b>${turma.professor.nome}<br /> <b><fmt:message
						key="modalidade" />: </b>${turma.modalidade.descricao}<br /> <b><fmt:message
						key="horarioInicial" />: </b>${turma.horarioInicial}<br /> <b><fmt:message
						key="horarioFinal" />: </b>${turma.horarioFinal}<br />
			</div>
		</div>
		<!-- module -->
		<div style="clear: both;"></div>
	...
	<div class="bottom-spacing">
		...
		<div class="float-right">
			<a
				...
				class="button"> <span><b><fmt:message key="new" /></b> <img
					...
					<tbody>
						<c:forEach var="aluno" items="${alunos}" varStatus="i"
							...
							<tr>
								...
								<td><a
									href="${pageContext.request.contextPath}/view/admin/aluno/alteraAluno.do?codigo=${aluno.idPessoa }"><img
	...
	<div class="pagination">
		<a
			href="${pageContext.request.contextPath}/view/admin/aluno/listaAlunos.do?pag=1"
			...
			href="${pageContext.request.contextPath}/view/admin/aluno/listaAlunos.do?pag=<c:if test="${pag > 1}">${pag - 1}</c:if><c:if test="${pag == 1}">1</c:if>"
			class="button"><span> <img
		...
		<div class="numbers">
			...
			<c:forEach var="i" begin="1" end="${numPags}">
				...
				<c:if test="${i != pag}">
					<a
						href="${pageContext.request.contextPath}/view/admin/aluno/listaAlunos.do?pag=${i}"><c:out
							value="${i}" /></a>
				</c:if>
				<c:if test="${i < numPags}">
					<span>|</span>
		...
		<a
			href="${pageContext.request.contextPath}/view/admin/aluno/listaAlunos.do?pag=<c:if test="${pag < numPags}">${pag + 1}</c:if> <c:if test="${pag == numPags}">${pag}</c:if>"
			...
			href="${pageContext.request.contextPath}/view/admin/aluno/listaAlunos.do?pag=<c:out value="${numPags}"/>"
