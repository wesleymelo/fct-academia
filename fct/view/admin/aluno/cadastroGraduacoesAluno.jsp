...
<div class="container_12">
	...
	<div class="grid_6">
		...
		<div class="module">
			<h2>
				<span><fmt:message key="inserirGraducaoAluno" /></span>
			</h2>

			<div class="module-body">

				<form action="cadastroGraduacaoAluno.do" method="post">
					<input type="hidden" name="idGraduacao" id="idGraduacao" /> <input
						type="hidden" name="idAluno" id="idAluno" value="${param.id}" />

					<p>
						<label><fmt:message key="graduacao" /></label> <input type="text"
							value="${param.graduacao}" class="input-short" name="graduacao"
							id="graduacao" readonly="readonly" value="${graduacao}" /><a
							href="javascript:abrir('buscaGraduacoesAluno.do')"> <fmt:message
								key="buscar" />
						</a>

						<c:if test="${not empty erros['errograduacao'] }">
							<span class="notification-input ni-error"><fmt:message
									key="graduacao_invalida" /></span>
						</c:if>

						<c:if test="${not empty erros['errograduacao_duplicada'] }">
							<span class="notification-input ni-error"><fmt:message
									key="graduacao_duplicada" /></span>
						</c:if>
					</p>
	...
	<div class="grid_6">
		...
		<div class="module">
			<h2>
				<span><fmt:message key="aluno" /></span>
			</h2>

			<div class="module-body">
					<b> <fmt:message key="nome" />: </b>${aluno.nome} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
					<b><fmt:message key="nascimento" />: </b>${aluno.dateNascimentoString}<br /> 
					<b> <fmt:message key="cpf" />: </b>${aluno.cpf} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><fmt:message key="sexo" />: </b>${aluno.sexo.descricao}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<!-- Example table -->
		<div class="module">
			<h2>
				<span><fmt:message key="graduacoes" /></span>
			</h2>
					...
					<table id="myTable" class="tablesorter">
						<thead>
							<tr>
								...
								<th style="width: 20%"><fmt:message key="modalidade"/></th>
								<th style="width: 10%"><fmt:message key="acoes"/></th>
							</tr>
						</thead>
						<tbody>
							<c:forEach var="gradu" items="${graduacoes}" varStatus="i" begin="${inicio}" end="${fim}" >
								<tr>
									<td><a href="">${gradu.descricao}</a></td>
									<td><a href="">${gradu.modalidadeString}</a></td>
									<td>	<a href="${pageContext.request.contextPath}/view/admin/graduacao/alteraGraduacao.do?codigo=${gradu.idGraduacao }"><img
		...
		<c:if test="${fn:length(graduacoes) % tamPag == 0}">
			<c:set var="numPags" value="${fn:length(graduacoes) / tamPag }" />
		</c:if>
		<c:if test="${fn:length(graduacoes) % tamPag != 0}">
			<c:set var="numPags" value="${(fn:length(graduacoes) / tamPag)+1}" />
		</c:if>

		<div class="pagination">
			<a href="${pageContext.request.contextPath}/view/admin/aluno/listaGraduacoesAluno.do?pag=1" class="button"><span><img
			...
			<a href="${pageContext.request.contextPath}/view/admin/aluno/listaGraduacoesAluno.do?pag=<c:if test="${pag > 1}">${pag - 1}</c:if><c:if test="${pag == 1}">1</c:if>"
					class="button"><span>
			...
			<div class="numbers">
				...
				<c:forEach var="i" begin="1" end="${numPags}">
					...
					<c:if test="${i != pag}">
						<a href="${pageContext.request.contextPath}/view/admin/aluno/listaGraduacoesAluno.do?pag=${i}"><c:out value="${i}" /></a>
					</c:if>
					<c:if test="${i < numPags}">
						<span>|</span>
					</c:if>	
			...
			<a href="${pageContext.request.contextPath}/view/admin/aluno/listaGraduacoesAluno.do?pag=<c:if test="${pag < numPags}">${pag + 1}</c:if> <c:if test="${pag == numPags}">${pag}</c:if>" class="button"><span><fmt:message key="proximo"/> <img
					src="${pageContext.request.contextPath}/view/images/arrow-000-small.gif"
					height="9" width="12" alt="Next" /></span>
			</a> 
					
			<a href="${pageContext.request.contextPath}/view/admin/aluno/listaGraduacoesAluno.do?pag=<c:out value="${numPags}"/>" class="button last"><span><fmt:message key="ultimo"/><img
