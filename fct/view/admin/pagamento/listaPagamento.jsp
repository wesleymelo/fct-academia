	...
	<div class="grid_12">
		...
		<div class="bottom-spacing">
			...
			<div class="float-right">
				<a
					href="${pageContext.request.contextPath}/view/admin/pagamento/cadastroPagamento.jsp"
			...
			<form action="buscaPagamento.do" method="post">
					<fieldset>
		...
		<div class="module">
			<h2>
				<span><fmt:message key="pagamentos" /></span>
			</h2>
			...
			<div class="module-table-body">
				<form action="">
					<table id="myTable" class="tablesorter">
						...
						<tbody>
							<c:forEach var="pagamento" items="${pagamentos}" varStatus="i" begin="${inicio}" end="${fim}">
								<tr>
									<td><a href="">${pagamento.valorPago}</a></td>
									<td><a href="">${pagamento.dataPagamentoString}</a></td>
									<td>	<a href="${pageContext.request.contextPath}/view/admin/pacote/alteraPamgamento.do?codigo=${pacote.idPagamento }"><img
		...
		<c:if test="${fn:length(pagamentos) % tamPag == 0}">
			<c:set var="numPags" value="${fn:length(pagamentos) / tamPag }" />
		</c:if>
		<c:if test="${fn:length(pagamentos) % tamPag != 0}">
			<c:set var="numPags" value="${(fn:length(pagamentos) / tamPag)+1}" />
		</c:if>


		<div class="pagination">
			<a href="${pageContext.request.contextPath}/view/admin/pagamento/listaPagamentos.do?pag=1" class="button"><span><img
			...
			<a href="${pageContext.request.contextPath}/view/admin/pagamento/listaPagamentos.do?pag=<c:if test="${pag > 1}">${pag - 1}</c:if><c:if test="${pag == 1}">1</c:if>"
					class="button"><span>
			...
			<a href="${pageContext.request.contextPath}/view/admin/pagamento/listaPagamentos.do?pag=<c:if test="${pag < numPags}">${pag + 1}</c:if> <c:if test="${pag == numPags}">${pag}</c:if>" class="button"><span><fmt:message key="proximo"/> <img
					src="${pageContext.request.contextPath}/view/images/arrow-000-small.gif"
					height="9" width="12" alt="Next" /></span>
			</a> 
					
			<a href="${pageContext.request.contextPath}/view/admin/pagamento/listaPagamentos.do?pag=<c:out value="${numPags}"/>" class="button last"><span><fmt:message key="ultimo"/><img
