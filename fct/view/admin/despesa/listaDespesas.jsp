	...
	<div class="grid_12">
		...
		<div class="bottom-spacing">
			...
			<div class="float-right">
				<a
					href="${pageContext.request.contextPath}/view/admin/despesa/cadastroDespesa.jsp"
			...
			<form action="buscaDespesas.do" method="post">
				<fieldset>
		...
		<div class="module">
			<h2>
				<span><fmt:message key="despesas" /></span>
			</h2>
					...
					<table id="myTable" class="tablesorter">
						<thead>
							<tr>
								...
								<th style="width: 20%"><fmt:message key="qtde"/></th>
								<th style="width: 10%"><fmt:message key="acoes"/></th>
							</tr>
						</thead>
						<tbody>
							<c:forEach var="despesa" items="${despesas}" varStatus="i" begin="${inicio}" end="${fim}">
								<tr>
									<td>${despesa.descricao}</td>
									<td>${despesa.quantidade}</td>
									<td>	 <a href="${pageContext.request.contextPath}/view/admin/despesa/alteraDespesa.do?codigo=${despesa.idDespesa }"><img
		...
		<c:if test="${fn:length(despesas) % tamPag == 0}">
			<c:set var="numPags" value="${fn:length(despesas) / tamPag }" />
		</c:if>
		<c:if test="${fn:length(despesas) % tamPag != 0}">
			<c:set var="numPags" value="${(fn:length(despesas) / tamPag)+1}" />
		</c:if>

		<div class="pagination">
			<a href="${pageContext.request.contextPath}/view/admin/despesa/listaDespesas.do?pag=1" class="button"><span><img
			...
			<a href="${pageContext.request.contextPath}/view/admin/despesa/listaDespesas.do?pag=<c:if test="${pag > 1}">${pag - 1}</c:if><c:if test="${pag == 1}">1</c:if>"
					class="button"><span>
			...
			<div class="numbers">
				...
				<c:forEach var="i" begin="1" end="${numPags}">
					...
					<c:if test="${i != pag}">
						<a href="${pageContext.request.contextPath}/view/admin/despesa/listaDespesas.do?pag=${i}"><c:out value="${i}" /></a>
					</c:if>
					<c:if test="${i < numPags}">
						<span>|</span>
					</c:if>	
			...
			<a href="${pageContext.request.contextPath}/view/admin/despesa/listaDespesas.do?pag=<c:if test="${pag < numPags}">${pag + 1}</c:if> <c:if test="${pag == numPags}">${pag}</c:if>" class="button"><span><fmt:message key="proximo"/> <img
					src="${pageContext.request.contextPath}/view/images/arrow-000-small.gif"
					height="9" width="12" alt="Next" /></span>
			</a> 
					
			<a href="${pageContext.request.contextPath}/view/admin/despesa/listaDespesas.do?pag=<c:out value="${numPags}"/>" class="button last"><span><fmt:message key="ultimo"/><img
